...
// This file has been modified to compile under the ZuluIDE project
...
bool xdvd_get_encrypted_challenge_table(uint8_t *xdvd_challenge_table_encrypted)
{
    ...
    FsFile f = SD.open(dvd_security_path, O_RDONLY);
    if (!f.isOpen()) {
        return false;
    }
    ...
    if (file_size == XDVD_STRUCTURE_LEN) {
        f.read(xdvd_challenge_table_encrypted, file_size);
        ...
        f.read(&xdvd_challenge_table_encrypted[4], 0x10);

        // Read challenge/response table onward to end of file
        f.seek(CR_ENTRIES_OFFSET - 6);
        f.read(&xdvd_challenge_table_encrypted[CR_ENTRIES_OFFSET - 2],  XDVD_STRUCTURE_LEN - (CR_ENTRIES_OFFSET - 2));
    } else {
        status = false;
    }
...
void xdvd_get_default_security_page(XBOX_DVD_SECURITY *xdvd_security)
{
    ...
    static const XBOX_DVD_SECURITY s = {
        ...
        //header
        {
            {0, sizeof(XBOX_DVD_SECURITY) - 2}, // ModeDataLength[2]
            0, // MediumType
            0, // DeviceSpecificParameter
            {0}, // Reserved[2]
            {0} // BlockDescriptorLength[2]
        },
        // page 
        {
            MODE_PAGE_XBOX_SECURITY, // PageCode
            sizeof(XBOX_DVD_SECURITY_PAGE) - 2, // PageLength
            0, // Partition: 0 - video, 1 - xbox
            1, // Unk1;
            0, // Authenticated;
            0xD1,// BookTypeAndVersion;
            2, // Unk2;
            0, // ChallengeID;
            0, // ChallengeValue;
            0, // ResponseValue;
