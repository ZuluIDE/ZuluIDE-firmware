...
bool IDEATAPIDevice::atapi_read(const uint8_t *cmd)
{
    ...
    {
        logmsg("-- Host attempted read at LBA ", (uint64_t)lba, "+", (uint64_t)transfer_len,
            ", beyond capacity ", (uint64_t)capacity_lba());
        return atapi_cmd_error(ATAPI_SENSE_ILLEGAL_REQ, ATAPI_ASC_LBA_OUT_OF_RANGE);
    }

    dbgmsg("-- Read ", (uint64_t)transfer_len, " sectors starting at ", (uint64_t)lba);
...
bool IDEATAPIDevice::atapi_write(const uint8_t *cmd)
{
    ...
    if (lba + transfer_len > capacity_lba())
    {
        logmsg("-- Host attempted write at LBA ", (uint64_t)lba, "+", (uint64_t)transfer_len,
            ", beyond capacity ", (uint64_t)capacity_lba());
        return atapi_cmd_error(ATAPI_SENSE_ILLEGAL_REQ, ATAPI_ASC_LBA_OUT_OF_RANGE);
    }

    dbgmsg("-- Write ", (uint64_t)transfer_len, " sectors starting at ", (uint64_t)lba);
...
bool IDEATAPIDevice::doWrite(uint32_t lba, uint32_t transfer_len)
{
    ...
    else if (m_atapi_state.crc_errors > 0)
    {
        logmsg("-- Detected ", m_atapi_state.crc_errors, " CRC errors during write to LBA ",
            (uint64_t)lba, ", reporting error to host");
        return atapi_cmd_error(ATAPI_SENSE_HARDWARE_ERROR, ATAPI_ASC_CRC_ERROR);
    }
    else
    {
