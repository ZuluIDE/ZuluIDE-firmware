...
void IDECDROMDevice::poll()
{

}

bool IDECDROMDevice::handle_command(ide_phy_msg_t *msg)
{
...
void IDECDROMDevice::handle_event(ide_phy_msg_t *msg)
{
...
bool IDECDROMDevice::cmd_identify_packet_device(ide_phy_msg_t *msg)
{
    uint16_t idf[256] = {0};
    volatile ide_msg_status_t status = IDE_MSGSTAT_IDLE;

    idf[IDE_IDENTIFY_OFFSET_GENERAL_CONFIGURATION] = 0x8580; // ATAPI CDROM device
...
bool IDECDROMDevice::cmd_packet(ide_phy_msg_t *msg)
{
    // Host gives limit to bytecount in responses
    m_bytes_req = msg->payload.cmd_start.lbamid | ((uint16_t)msg->payload.cmd_start.lbahigh << 8);
...
bool IDECDROMDevice::set_packet_device_signature(uint8_t error)
{
    ide_phy_msg_t msg = {};
...
bool IDECDROMDevice::atapi_send_data(const uint16_t *data, uint16_t byte_count)
{
    volatile ide_msg_status_t status = IDE_MSGSTAT_IDLE;
...
{
    azdbg("-- ATAPI command: ", cmd[0], " ", get_atapi_command_name(cmd[0]));
...
bool IDECDROMDevice::atapi_cmd_error(uint8_t sense_key, uint16_t sense_asc)
{
    azdbg("-- Reporting ATAPI error: ", sense_key, " ", sense_asc);
    m_sense_key = sense_key;
    m_sense_asc = sense_asc;
...
bool IDECDROMDevice::atapi_cmd_ok()
{
...
bool IDECDROMDevice::atapi_test_unit_ready(const uint8_t *cmd)
{
    return atapi_cmd_ok();
}

bool IDECDROMDevice::atapi_inquiry(const uint8_t *cmd)
{
    uint8_t req_bytes = cmd[4];
    uint16_t inquiry_buf[18] = {0};
    uint8_t *inquiry = (uint8_t*)inquiry_buf;
    uint8_t count = 36;
    inquiry[ATAPI_INQUIRY_OFFSET_TYPE] = 0x05; // CD-ROM device
    inquiry[ATAPI_INQUIRY_REMOVABLE_MEDIA] = 0x80;
